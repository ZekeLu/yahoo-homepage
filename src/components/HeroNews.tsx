...
export default function HeroNews() {
  return (
    <section aria-label="Top stories" className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <h2 className="mb-4 text-xl font-bold text-gray-900">Top Stories</h2>
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Hero card */}
        <article className="lg:col-span-2 overflow-hidden rounded-lg bg-white shadow-md">
          <div className="h-56 w-full bg-gradient-to-br from-yahoo-purple to-yahoo-purple-light flex items-center justify-center sm:h-72">
            <span className="text-5xl" role="img" aria-label={heroArticle.imageAlt}>
              🌍
            </span>
          </div>
          <div className="p-5">
            ...
            <p className="mt-2 text-sm text-gray-600">{heroArticle.summary}</p>
          </div>
        </article>
        ...
        <div className="flex flex-col gap-3">
          {sideArticles.map((article) => (
            <article
              key={article.title}
              className="flex items-start gap-3 rounded-lg bg-white p-4 shadow-sm hover:shadow-md transition-shadow"
            >
